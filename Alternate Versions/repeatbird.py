...
FRAME_CHECK = 12
killed = False
previousStateAction = ["00", "nothing"]
prevHoriz = 0
prevVert = 0
pipeSet = []
pipeCount = 0
...
class PipePair(pygame.sprite.Sprite):
    ...
    total_pipe_body_pieces = int(
        ...
        PIECE_HEIGHT          # to get number of pipe pieces
    )
    ...
    def __init__(self, pipe_end_img, pipe_body_img):
        ...
        self.bottom_pieces = pipeSet[pipeCount] # 5 #randint(1, total_pipe_body_pieces)
        pipeCount += 1
        if(pipeCount >= len(pipeSet)):
            pipeCount = 0
        self.top_pieces = PipePair.total_pipe_body_pieces - self.bottom_pieces
...
class QMatrix(object):
    """docstring for """
    def __init__(self, verticalDist, horizontalDist, riseFall, groundDist):

        if(os.path.isfile("qFile")):
            qFile = open("qFile", "r")
            self.matrix = ast.literal_eval(qFile.read())
            qFile.close()

        self.matrix = [[], [], []]
        self.matrix[0].append(0)
        self.matrix[1].append("jump")
        self.matrix[2].append("nothing")

        self.matrix[0].append(str(verticalDist) + str(horizontalDist) + riseFall + str(groundDist))
    ...
    def get(self, state, action):
        """Returns the value of the given state-action pair.
           ...
           If this is a new pair, adds it to the matrix and
           returns 0."""
        xVal = -1;
        yVal = -1
        for i in range(0, len(self.matrix)):
            if(self.matrix[i][0] == action):
                xVal = i;
                break;
        for i in range(0, len(self.matrix[0])):
            if(self.matrix[0][i] == state):
                yVal = i;
        if(yVal == -1):
            self.matrix[0].append(state)
            self.matrix[1].append(0)
            self.matrix[2].append(0)
            return 0;
        return self.matrix[xVal][yVal];

    def add(self, state, action, value):
        """Add the given Q-value to the given state-action pair."""
        xVal = -1;
        yVal = -1;
        for i in range(0, len(self.matrix)):
            if(self.matrix[i][0] == action):
...
def approximate(num):
    return 5 * round(float(num)/5)

def computeRVal(horizDist, prevAction):
    global killed
    #print(horizDist)
    if(killed):
        killed = False
        return -1000
    elif(horizDist < -50 and prevAction == "nothing"):
        return -1
    elif(horizDist > 150 and prevAction == "jump"):
        return -1
    else:
        return 1
...
def main():
    ...
    seed(0)
    for i in range(0, 10):
        pipeSet.append(randint(1, PipePair.total_pipe_body_pieces))

    clock = pygame.time.Clock()
    score_font = pygame.font.SysFont(None, 32, bold=True)  # default font
    images = load_images()
    qStates = QMatrix(0, 0, "fall", 0)
    while True:
        ...
        while not done:
            ...
            if(frame_clock % FRAME_CHECK == 0):
                ezBirdX = bird.x + Bird.WIDTH
                ezBirdY = WIN_HEIGHT - bird.y + Bird.HEIGHT
                vertDist = approximate(ezBirdY - pp.bottom_height_px)
                horizDist = approximate(ezBirdX - pp.x)
                riseFall = ""
                if(bird.msec_to_climb > 0):
                    riseFall = "rise"
                else:
                    riseFall = "fall"
                groundDist = approximate(WIN_WIDTH - ezBirdY)

                currentState = str(vertDist) + str(horizDist) + riseFall + str(groundDist)

                print(currentState)

                prevState = previousStateAction[0]
                prevAction = previousStateAction[1]

                #print(prevState)
                #print(prevAction)
                qPrev = qStates.get(prevState, prevAction)
                rPrev = computeRVal(prevVert, prevAction)
                prevVert = vertDist

                qCurrentNothing = qStates.get(currentState, "nothing")
                qCurrentJump = qStates.get(currentState, "jump")

                if(qCurrentNothing < qCurrentJump):
                    qCurrent = qCurrentJump
                else:
                    qCurrent = qCurrentNothing

                qNothing = qStates.get(str(vertDist) + str(horizDist), "nothing")
                qJump = qStates.get(str(vertDist) + str(horizDist), "jump")

                action = ""

                if(qCurrentJump > qCurrentNothing):
                    action = "jump"
                else:
                    action = "nothing"
                ...
                qPrev = qPrev + ALPHA * (rPrev + GAMMA * qCurrent - qPrev)

                qStates.add(prevState, prevAction, qPrev)

                previousStateAction[0] = currentState
                previousStateAction[1] = action
        ...
        pipeCount = 0
        qFile = open("qFile", "w")
        qFile.write(str(qStates.matrix))
        qFile.close
    pygame.quit()
